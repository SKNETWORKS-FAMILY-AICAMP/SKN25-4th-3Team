import random
from langchain_openai import ChatOpenAI
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.output_parsers import StrOutputParser

# 장고 앱 구조에 맞게 수정된 임포트 경로
from recipes.utils.config import OPENAI_API_KEY
from recipes.rag.search_engine import (
    search_naver_blogs, 
    fetch_blog_body, 
    clean_html, 
    resolve_blog_url
)

# 1. 메뉴 선정을 위한 프롬프트
DIET_SELECTION_PROMPT = """
당신은 최고의 다이어트 식단 전문가입니다. 
아래 검색 결과를 참고하여 오늘 사용자에게 추천할 만한 '구체적인 다이어트 요리명' 하나를 선정해 주세요.
반드시 요리 이름만 짧게 답변해 주세요. (예: 두부면 팟타이)

[검색 결과 목록]
{titles}
"""

# 2. 레시피 요약을 위한 프롬프트
DIET_RECIPE_SUMMARY_PROMPT = """
당신은 친절한 요리사입니다. 아래의 블로그 본문을 읽고, '{dish_name}'에 대한 레시피 노트를 작성해 주세요.

[블로그 본문]
{body}

작성 규칙:
1. 오직 제공된 본문 내용을 바탕으로 **[필요한 재료]**와 **[조리 순서]**만 요약해 주세요.
2. **칼로리나 수치 정보는 불확실할 수 있으므로 절대 언급하지 마세요.**
3. 친절하고 부드러운 말투(~해요, ~하세요)를 사용해 주세요.
4. 너무 길지 않게 핵심 조리 과정만 요약해 주세요.
"""

class DietRecipeCurator:
    def __init__(self):
        self.llm = ChatOpenAI(
            openai_api_key=OPENAI_API_KEY,
            model="gpt-4o-mini",
            temperature=0.7
        )
        self.parser = StrOutputParser()

    def get_automatic_diet_recipe(self) -> dict:
        """자동으로 다이어트 레시피를 큐레이션하고 요약까지 제공합니다."""
        
        print("🥗 [Diet Curator] 1단계: 최신 다이어트 트렌드 탐색 중...")
        # Step 1: 트렌드 탐색
        search_queries = ["다이어트 레시피 추천", "인기 저칼로리 식단", "간단 다이어트 요리"]
        base_query = random.choice(search_queries)
        explore_items = search_naver_blogs(base_query, display=8, sort_type="date")
        
        if not explore_items:
            return {"answer": "현재 네이버에서 정보를 가져올 수 없습니다.", "source": "web"}

        # Step 2: 메뉴 확정
        titles_str = "\n".join([clean_html(i['title']) for i in explore_items])
        selection_chain = ChatPromptTemplate.from_template(DIET_SELECTION_PROMPT) | self.llm | self.parser
        selected_dish = selection_chain.invoke({"titles": titles_str}).strip()
        print(f"💡 [Diet Curator] AI 선정 메뉴: {selected_dish}")

        # Step 3: 상세 검색 및 본문 수집
        target_items = search_naver_blogs(f"{selected_dish} 레시피", display=3, sort_type="sim")
        if not target_items:
            return {"answer": f"'{selected_dish}' 레시피를 찾는 데 실패했습니다.", "source": "web"}

        best_item = target_items[0]
        body_text = fetch_blog_body(best_item['link'], max_chars=1500)
        
        # Step 4: 레시피 요약 생성
        print(f"📝 [Diet Curator] 2단계: '{selected_dish}' 레시피 요약 중...")
        if body_text and len(body_text) > 200:
            summary_prompt = ChatPromptTemplate.from_template(DIET_RECIPE_SUMMARY_PROMPT)
            summary_chain = summary_prompt | self.llm | self.parser
            recipe_summary = summary_chain.invoke({"dish_name": selected_dish, "body": body_text})
        else:
            recipe_summary = f"본문 내용을 충분히 가져오지 못해 요약이 어렵습니다. 아래 링크를 통해 직접 확인해 보세요! 😊"

        # 최종 응답 구성
        final_answer = (
            f"🥗 **오늘 당신을 위한 추천 다이어트 요리는 '{selected_dish}'입니다!**\n\n"
            f"{recipe_summary}\n\n"
            f"---\n"
            f"더 자세한 사진과 설명은 아래 블로그 링크를 참고해 보세요! 👇"
        )

        candidates = [{
            "title": f"[레시피] {clean_html(best_item['title'])}",
            "url": resolve_blog_url(best_item['link']),
            "ingredients_summary": f"'{selected_dish}' 추천 레시피 정보",
            "source": "web"
        }]

        return {"answer": final_answer, "source": "web", "candidates": candidates}
